...
int process_function_def(char *buffer, FILE *file2)
{
               ...
               output.output_data_12))
    {
        fprintf(file2, "%s %s(%s %s, %s %s) {\n",
                ...
                output.output_data_13,
                output.output_data_11,
                output.output_data_10);printf("hello world!");
        return 0;
    }
...
int main(int argc, char *argv[])
{
    ...
    while (fgets(buffer, sizeof(buffer), file1) != NULL)
    {
        ...
        if (process_function_def(buffer, file2))
        {
            ...
            process_math_def(buffer, file2);
        }
    }
    char command[256];
    snprintf(command, sizeof(command), "gcc -g -o %s %s", argv[3], argv[2]);
    system(command);
